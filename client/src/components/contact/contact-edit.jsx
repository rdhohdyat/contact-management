...
import { contactDetail, contactUpdate } from "../../lib/api/contact-api";
...
export default function ContactEdit() {
  ...
  const fetchContact = async () => {
    ...
    if (response.status == 200) {
      ...
      alertError("contact is not found");
    }
  };
  ...
  const handleContactSubmit = async (e) => {
    ...
    const response = await contactUpdate(token, id, {
      first_name,
      last_name,
      email,
      phone,
  ...
  useEffectOnce(() => {
    fetchContact().then(() => console.log("success get detail contact"));
  });
